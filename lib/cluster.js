...
class Cluster {
  ...
  _getConn(opts) {
    ...
    console.log("JLLIS: key name: " + connKey + " // conn object = " + conn + " // closed? " + (!conn ? false : conn._closed));

    if (conn && conn._closed) {
      // Dead connection. Force new connection
      console.log("JLLIS: non-null conn but closed")
      conn = null;
    }
    ...
    if (!conn) {
      ...
      conn.connect().catch((e) => {
        console.log('JLLIS: failed to connect to bucket: %O', e);
        conn.close();
      });
